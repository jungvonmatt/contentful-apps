 ...
 */
export function useLooseFieldValue<Value = unknown>(
  fieldId?: string,
  locale?: string
): UseFieldValueReturnValue<Value> {
  const sdk = useSDK();
  const entryFieldApi = useMemo(
    () => getEntryFieldApi(sdk, fieldId),
    [sdk, fieldId]
  );
...
function getEntryFieldApi(
  ...
  const fieldIdWithDefault =
    fieldId ?? ("field" in sdk ? sdk.field.id : undefined);
  ...
  return sdk.entry.fields[fieldIdWithDefault];
}
